...
int main(int argc, char* argv[])
{
	...
	// Load texture
	const char * texFilename = "../../resources/textures/white_desk.jpg";
	...
	const char * texFilename2 = "../../resources/textures/aluminum.jpg";
	if (!UCreateTexture(texFilename2, gComputerTextureId))
	{
		cout << "Failed to load texture " << texFilename2 << endl;
		return EXIT_FAILURE;
	}

	const char * texFilename3 = "../../resources/textures/black.png";
	if (!UCreateTexture(texFilename3, gBlackTextureId))
	{
		cout << "Failed to load texture " << texFilename3 << endl;
		return EXIT_FAILURE;
	}

	const char * texFilename4 = "../../resources/textures/white.png";
	if (!UCreateTexture(texFilename4, gWhiteTextureId))
	{
...
void URender()
{
	...
	glUniform3f(light1PosLoc, 0.0f, 4.0f, -1.0f);
	glUniform3f(light2ColLoc, 0.4f, 0.4f, 0.4f);	
	glUniform3f(light2PosLoc, 1.0f, .5f, 4.0f);
	//set specular intensity
	glUniform1f(specInt1Loc, 1.3f);
	//set specular highlight size
	glUniform1f(highlghtSz1Loc, .2f);
	//set specular intensity
	glUniform1f(specInt2Loc, 3.0f);
	//set specular highlight size
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(2.0f, .1f, .75f));
	// 2. Rotates shape
	...
	glDrawElements(GL_TRIANGLE_STRIP, meshes.gPlaneMesh.nIndices, GL_UNSIGNED_INT, (void*)0);
	glBindVertexArray(0);
	...
	////////////////////////////////
	// 1. Scales the object
	scale = glm::scale(glm::vec3(.25f, .005f, 0.20f));
	// 2. Rotates shape
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. translate shape
	translation = glm::translate(glm::vec3(0.0f, 0.01f, 0.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	glUniform2f(glGetUniformLocation(gSurfaceProgramId, "uvScale"), .5f, .5f);

	// bind textures on corresponding texture units
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, gComputerTextureId);

	glDrawArrays(GL_TRIANGLE_STRIP, 0, meshes.gCubeMesh.nVertices);

	////////////////////////////////
	// render the vertical support
	////////////////////////////////
	// 1. Scales the object
	scale = glm::scale(glm::vec3(.25f, .3f, 0.005f));
	// 2. Rotates shape
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. translate shape
	translation = glm::translate(glm::vec3(0.0f, 0.15f, -0.10f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	glUniform2f(glGetUniformLocation(gSurfaceProgramId, "uvScale"), .5f, .5f);
	...
	////////////////////////////////
	// render the computer monitor
	////////////////////////////////
	// 1. Scales the object
	scale = glm::scale(glm::vec3(1.2f, .85f, 0.08f));
	// 2. Rotates shape
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. translate shape
	translation = glm::translate(glm::vec3(0.0f, 0.65f, -0.06f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	glUniform2f(glGetUniformLocation(gSurfaceProgramId, "uvScale"), .5f, .5f);
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(.6f, .002f, 0.36f));
	// 2. Rotates shape
	rotation = glm::rotate(1.582f, glm::vec3(1.0, 0.0f, 0.0f));
	// 3. translate shape
	translation = glm::translate(glm::vec3(0.0f, 0.715f, -0.017f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	glUniform2f(glGetUniformLocation(gSurfaceProgramId, "uvScale"), 1.0f, 1.0f);
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(.55f, .002f, 0.28f));
	// 2. Rotates shape
	rotation = glm::rotate(1.582f, glm::vec3(1.0, 0.0f, 0.0f));
	// 3. translate shape
	translation = glm::translate(glm::vec3(0.0f, 0.76f, -0.015f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	glUniform2f(glGetUniformLocation(gSurfaceProgramId, "uvScale"), .5f, .5f);
	...
	// bind textures on corresponding texture units
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, gWhiteTextureId);
	...
	scale = glm::scale(glm::vec3(1.6f, .02f, .5f));
	...
	// 2. Rotates shape by 15 degrees in the x axis
	rotation = glm::rotate(1.3f, glm::vec3(1.0, 0.0f, 0.0f));
	// 3. Place object at the origin
	translation = glm::translate(glm::vec3(1.0f, .5f, 4.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
